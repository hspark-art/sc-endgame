...
#
# 아래 값은 2026-09-16 실측입니다 — 두 채널(@ETALENT-SC 978개·@ETALENT-TV 389개)의
# 최근 180일 업로드 112개를 KST 시각별로 세어 나온 결과:
#     11시 8 · 12시 33 · 13시 16 · 14시 5 · 15시 15 · 16시 11 · 17시 14 · 18시 6
#     (그 밖: 07시 1 · 20시 1 · 21시 2)
# 낮 11~18시에 96%(108/112)가 몰립니다. 그래서 그 여덟 시간만 봅니다
# — 하루 8번, 한 번에 30 units 쯤이라 하루 240 (할당량 10,000).
# 드물게 밤에 올라온 영상도 영영 놓치지는 않습니다: 조회할 때마다 채널 전체를
# 다시 훑으므로, 다음 날 11시에 붙습니다(늦어질 뿐입니다).
#
# 비워 두면(= ()) '하루 한 번'으로 돕니다. 다시 재려면:
#       python3 tools/fetch_videos.py --when       (PC 에서. API 키가 있어야 합니다)
#   최근 180일 분포와 넣을 값을 그대로 알려 줍니다. 자동 갱신 로그(Actions)에도
#   조회할 때마다 같은 표가 찍히니 거기서 보셔도 됩니다.
