...
## 승부예측 이벤트 (2026-08-21 추가)

시청자가 채팅으로 세트 승자를 맞히면 포인트가 쌓이는 이벤트입니다.

- **규칙**: 관제에서 예측 시작 → 시청자가 채팅에 **선수 이름**(첫 입력만
  인정, 변경 불가) → 마감 → 세트 끝나면 결과 버튼으로 정산.
  적중 **+100P**, 연승 보너스 **+20P씩**(최대 +100P). 틀리면 연승만 리셋.
- **화면**: 관제 `admin/prize.php`의 🔮 승부예측 패널(시작/마감/결과/취소,
  실시간 비율바) · 방송 장면 `prize_overlay.php`(kind=predict 실시간 비율,
  kind=predict_result 결과+상위 획득자) · **공개 리더보드 `predict.php`**
  (사이트 루트, 로그인 불필요, 10초 갱신, 닉네임 검색).
- **저장**(서버 `admin/pz/`): `predict_cur.json`(진행 라운드, 4초마다 저장 —
  관제 새로고침해도 이어짐) · `predict_points.json`(누적 포인트, 계정 기준)
  · `predict_rounds.json`(최근 60라운드). **정산은 서버(prize_api.php의
  predict_settle)가 하고, 서버에 같은 라운드가 있을 때만 처리해 두 번
  눌러도 중복 지급되지 않습니다.** 연습 모드(다른 채널/?demo)는 서버에
  안 쓰고 관제 안에서만 휘발성으로 돕니다.
- **prize_api.php 는 predict_public(공개 리더보드 읽기, 닉네임만 노출)
  한 액션만 로그인 없이 허용**하고 나머지는 전부 로그인 필수 그대로입니다.
